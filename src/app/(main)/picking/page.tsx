...
import { useAggregatedData, hourlySlots, getSlot } from "@/lib/data-context";
...
export default function PickingPage() {
  const { chartData, totalPicking, pickingData, totalPickingTOs } = useAggregatedData();
  const [selectedOperators, setSelectedOperators] = useState<Set<string>>(new Set());

  // Recent picking activity
  ...
  // Operator grouped chart data (TO count per operator per hour)
  const { operatorChartData, operators } = useMemo(() => {
    const map = new Map<string, any>();
    hourlySlots.forEach(slot => {
      map.set(`${slot.start} - ${slot.end}`, { time: slot.start });
    });
    ...
    pickingData.forEach(p => {
      ...
      if (slot === "Mimo směnu" || !map.has(slot)) return;

      if (!opTOs.has(slot)) opTOs.set(slot, new Map());
      const slotMap = opTOs.get(slot)!;
      if (!slotMap.has(p.operator)) slotMap.set(p.operator, new Set());
      slotMap.get(p.operator)!.add(p.to_number);
    });

    const ops = new Set<string>();
    opTOs.forEach((slotMap, slot) => {
    ...
    return {
      operatorChartData: Array.from(map.values()),
  ...
  const visibleOperators = selectedOperators.size > 0 ? operators.filter(o => selectedOperators.has(o)) : operators;
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="text-3xl font-black text-white">{totalPickingTOs.toLocaleString()}</div>
        ...
        <div className="glass-panel p-6">
          <div className="text-3xl font-black text-white">{totalPickingTOs > 0 ? Math.round(totalPicking / totalPickingTOs) : 0}</div>
        ...
        <div className="h-[320px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="pickAreaGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#6391ff" stopOpacity={0.2}/>
      ...
      {/* Graf 2: Skupinový graf operátorů (TO) s filtrem */}
      <div className="glass-panel p-6">
            ...
            {operators.map((op, i) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                  selectedOperators.size === 0 || selectedOperators.has(op)
                    ? 'border-white/20 text-white bg-white/10'
                    : 'border-white/5 text-white/25 bg-transparent'
                }`}
                style={{ borderColor: selectedOperators.has(op) ? OPERATOR_COLORS[i % OPERATOR_COLORS.length] : undefined }}
              >
                <span className="inline-block w-2 h-2 rounded-full mr-1.5" style={{ backgroundColor: OPERATOR_COLORS[i % OPERATOR_COLORS.length] }} />
                {op}
              </button>
            ))}
          </div>
      ...
      {/* Employee Performance - filtered to picking only */}
      <EmployeePerformance timeRange="daily" filterType="picking" />

      {/* Recent Activity Table - MOVED BELOW Employee Performance */}
      <div className="glass-panel overflow-hidden">
        <div className="p-5 border-b border-white/5 bg-white/[0.02]">
          <h3 className="text-sm font-bold text-white/70 uppercase tracking-wider">Poslední Picking aktivity (LTAP)</h3>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                ...
                <th className="px-5 py-3.5 text-xs font-semibold text-white/40 uppercase tracking-wider text-right">Množství (Ks)</th>
            ...
            <tbody>
              {recentPicking.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-8 text-center text-white/30">Zatím nejsou k dispozici žádná data.</td></tr>
                ...
                <tr key={i} className={`hover:bg-white/[0.03] transition-colors ${i % 2 === 1 ? 'bg-white/[0.015]' : ''}`}>
                  ...
                  <td className="px-5 py-3 text-sm text-white/40 text-right">{new Date(row.confirmed_at).toLocaleTimeString('cs-CZ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
