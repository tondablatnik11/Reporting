    ...
    return (
      <div className="bg-[#0a0e1e]/95 border border-white/10 p-4 rounded-xl shadow-2xl min-w-[260px] backdrop-blur-md z-50">
        <p className="text-white font-bold mb-3 pb-2 border-b border-white/10 flex justify-between items-center">
          <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-blue-400"/> {data.label}</span>
          <span className="bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded text-xs font-bold">{data.total} TO</span>
        </p>
            ...
            <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-2">
              {data.operators.map((op: any, i: number) => (
                <div key={i} className="flex justify-between items-center text-xs">
                   ...
                   <span className="font-bold text-blue-400">{op.val} TO</span>
                </div>
              ))}
            </div>
          </>
...
export default function PickingPage() {
  ...
  const hourlyChartData = useMemo(() => {
    ...
    hourlyDetails.forEach(row => {
      ...
      const val = Number(row.pick_tos) || 0;
      const ks = Number(row.pick_qty) || 0;
                ...
                <div className="p-6">
                  {loadingHourly ? (
                    <div className="h-[350px] flex items-center justify-center gap-3 text-white/30">
                      <Loader2 className="w-6 h-6 animate-spin text-blue-400" /> Načítám hodinový log...
                    </div>
