...
import { PackageSearch, Users, Activity, AlertOctagon, Zap, Loader2, AlertCircle, Clock } from "lucide-react";
...
export default function PickingPage() {
  ...
  // Nový stav pro hodinový detail konkrétního dne
  ...
  // Načítání hodinového detailu pokud sledujeme konkrétní dny
  useEffect(() => {
  ...
  const loadData = async (range: TimeRange) => {
    ...
    try {
      ...
      const { data: dbData, error: dbError } = await supabase.rpc('get_picking_analytics_data', {
  ...
  const loadHourlyDetail = async (targetDate: string) => {
    ...
    try {
      const { data: hData, error: hError } = await supabase.rpc('get_picking_hourly_detail', {
        p_date: targetDate
      });
  ...
  const { chartData, stats, priorityStats, shiftStats, operatorLeaderboard } = useMemo(() => {
    let totalTOs = 0, totalKs = 0, nTo = 0, eTo = 0, oTo = 0, aTo = 0, bTo = 0;
    const opsSet = new Set<string>();
    const opsLeaderMap = new Map<string, { name: string, tos: number, ks: number }>();
    ...
    data.forEach(d => {
      ...
      if (!groupedMap.has(key)) {
        groupedMap.set(key, { 
            key, label, pickingTOs: 0, pickingKs: 0, 
      ...
      const norm = Number(d.pick_normal_tos);
      const exp = Number(d.pick_express_tos);
      const oe = Number(d.pick_oe_tos);

      g.pickingTOs += tos;
      g.pickingKs += ks;
      g.pickingNormal += norm;
      g.pickingExpress += exp;
      g.pickingOE += oe;
      ...
      if (!opsLeaderMap.has(d.operator)) opsLeaderMap.set(d.operator, { name: d.operator, tos: 0, ks: 0 });
      const le = opsLeaderMap.get(d.operator)!;
      le.tos += tos;
      le.ks += ks;

      const shiftAB = mapShiftNameToAB(d.report_date, d.shift_name);
      if (shiftAB === 'A') aTo += tos;
      else if (shiftAB === 'B') bTo += tos;
    });

    const fChartData = Array.from(groupedMap.values()).sort((a, b) => a.key.localeCompare(b.key));
    const pStats = [
      { name: 'Normal', value: nTo, pct: totalTOs ? (nTo/totalTOs)*100 : 0, color: PRIORITIES_COLORS.Normal },
      { name: 'Express', value: eTo, pct: totalTOs ? (eTo/totalTOs)*100 : 0, color: PRIORITIES_COLORS.Express },
      { name: 'OE', value: oTo, pct: totalTOs ? (oTo/totalTOs)*100 : 0, color: PRIORITIES_COLORS.OE },
    ].filter(x => x.value > 0);

    const sStats = [
      { name: 'Směna A', value: aTo, pct: (aTo+bTo) ? (aTo/(aTo+bTo))*100 : 0, color: SHIFT_COLORS.A },
      { name: 'Směna B', value: bTo, pct: (aTo+bTo) ? (bTo/(aTo+bTo))*100 : 0, color: SHIFT_COLORS.B },
    ].filter(x => x.value > 0);

    const opLead = Array.from(opsLeaderMap.values()).sort((a, b) => b.tos - a.tos).slice(0, 15);

    return { 
        chartData: fChartData, 
        stats: { totalTOs, totalKs, uniqueOperators: opsSet.size }, 
  ...
  return (
    <div className="space-y-6 animate-fade-in-up pb-10">
          ...
          <div className="flex items-center bg-white/5 p-1 rounded-xl border border-white/10">
            {[ {id: 'day', label: 'Dny'}, {id: 'week', label: 'Týdny'}, {id: 'month', label: 'Měsíce'} ].map(g => (
              <button key={g.id} onClick={() => setGrouping(g.id as Grouping)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${grouping === g.id ? 'bg-blue-500 text-white shadow-lg' : 'text-white/50 hover:text-white'}`}
              >
                {g.label}
              </button>
            ))}
      ...
      {data.length === 0 ? (
        <div className="glass-panel p-8 text-center space-y-4">
            <PackageSearch className="w-12 h-12 text-white/20 mx-auto" />
        ...
        <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div className="text-3xl font-black text-white">{stats.totalTOs.toLocaleString()}</div>
                <p className="text-sm font-medium text-white/40 mt-1">Transfer Ordery</p>
                </div>
                <div className="glass-panel p-6 border-l-4 border-l-blue-400/50">
                ...
                <div className="text-3xl font-black text-white">{stats.totalTOs > 0 ? (stats.totalKs / stats.totalTOs).toFixed(1) : "0"}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="w-40 h-40 shrink-0 relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><span className="text-xl font-black text-white">{stats.totalTOs}</span></div>
                </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="h-[280px] w-full mt-6">
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        <defs>
                            <linearGradient id="colorKs" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.5}/><stop offset="95%" stopColor="#3b82f6" stopOpacity={0.0}/></linearGradient>
                        </defs>
                        ...
                        <Area yAxisId="left" type="monotone" dataKey="pickingKs" name="Objem (Ks)" fill="url(#colorKs)" stroke="#3b82f6" strokeWidth={2} />
                        <Bar yAxisId="right" dataKey="pickingTOs" name="Transfer Ordery (TO)" fill="#60d4ff" radius={[2,2,0,0]} />
                        </ComposedChart>
                    </ResponsiveContainer>
                </div>
                </div>
                ...
                <div className="glass-panel p-6">
                <h3 className="text-lg font-bold text-white mb-1 flex items-center gap-2"><AlertOctagon className="w-5 h-5 text-blue-400" /> Rozpad zakázek podle typu (TO)</h3>
                <div className="h-[280px] w-full mt-6">
                    <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="glass-panel overflow-hidden border-t-4 border-t-blue-500">
                ...
                {loadingHourly ? (
                  <div className="p-8 text-center text-white/30 flex items-center justify-center gap-2">
                    <Loader2 className="w-4 h-4 animate-spin text-blue-400" /> Načítám podrobný hodinový log...
                  </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead className="sticky top-0 bg-[#121625] z-10 shadow-md">
                        <tr className="border-b border-white/5">
                          ...
                          <th className="px-5 py-3 text-xs font-semibold text-white/40 uppercase text-right">Odbaveno TO</th>
                      ...
                      <tbody>
                        {hourlyDetails.map((row, i) => (
                          <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                            <td className="px-5 py-2.5 text-sm font-bold text-blue-400">{row.hour_slot} - {String(Number(row.hour_slot.split(':')[0])+1).padStart(2,'0')}:00</td>
                            ...
                            <td className="px-5 py-2.5 text-sm text-right font-bold text-white">{row.pick_tos}</td>
                            <td className="px-5 py-2.5 text-sm text-right font-bold text-white/60">{Number(row.pick_qty).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="glass-panel p-6">
                <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2"><Users className="w-5 h-5 text-blue-400" /> Produktivita Pickerů (Top 15)</h3>
