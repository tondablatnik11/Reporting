...
export default function PackingPage() {
  const { chartData, totalPacking, packingData, totalPackingHUs } = useAggregatedData();
  ...
  // Operator grouped chart data (HU count per operator per hour)
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="text-3xl font-black text-white">{totalPackingHUs.toLocaleString()}</div>
        ...
        <div className="glass-panel p-6">
          <div className="text-3xl font-black text-white">{totalPackingHUs > 0 ? Math.round(totalPacking / totalPackingHUs) : 0}</div>
      ...
      {/* Graf 2: Skupinový graf operátorů (HU) s filtrem */}
      <div className="glass-panel p-6">
      ...
      {/* Employee Performance - filtered to packing only */}
      ...
      <div className="glass-panel overflow-hidden">
        <div className="p-5 border-b border-white/5 bg-white/[0.02]">
          <h3 className="text-sm font-bold text-white/70 uppercase tracking-wider">Poslední Packing aktivity (VEKP / VEPO)</h3>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {recentPacking.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-white/30">Zatím nejsou k dispozici žádná data.</td></tr>
                ...
                <tr key={i} className={`hover:bg-white/[0.03] transition-colors ${i % 2 === 1 ? 'bg-white/[0.015]' : ''}`}>
                  ...
                  <td className="px-5 py-3 text-sm text-white/40 text-right">{row.created_at ? new Date(row.created_at).toLocaleTimeString('cs-CZ') : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
